...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _performSearch(String origin, String dest, double? oLat, double? oLng, double? dLat, double? dLng, DateTime date, double radius) {
    setState(() {
...
class SearchTab extends StatefulWidget {
  final Function(String, String, double?, double?, double?, double?, DateTime, double) onSearch;
...
class _SearchTabState extends State<SearchTab> {
  ...
  final _dateDisplayController = TextEditingController(text: DateFormat('dd/MM/yyyy').format(DateTime.now()));
  ...
  final List<String> _smartZones = [
    "Storkøbenhavn", "Nordsjælland", "Vestsjælland", "Sydsjælland", "Midtsjælland",
    "Aarhus & Omegn", "Odense & Omegn", "Aalborg & Omegn", "Trekantsområdet",
  ];
  ...
  void _useZone(String zoneName) {
    setState(() {
      if (_isDestFieldActive) {
        ...
        _selOriginLat = null; _selOriginLng = null;
      }
    });
  }
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    const CircleAvatar(radius: 24, backgroundColor: Color(0xFFF1F5F9), child: Icon(Icons.person, color: Colors.grey))
                  ],
                ),
              ),
              Container(
                ...
                decoration: BoxDecoration(color: _bgLight, borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.grey.shade200)),
                child: Column(
                  children: [
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text("Radius: ${_currentRadius.round()} km", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: _primaryColor)),
                    ...
                    SingleChildScrollView(
                      ...
                      child: Row(
                        children: _smartZones.map((zone) => Padding(
                    ...
                    ListTile(
                      ...
                      title: const Text("Hvornår?", style: TextStyle(fontSize: 12, color: Colors.grey)),
                      subtitle: Text(_dateDisplayController.text, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: _primaryColor)),
                      onTap: () => _pickDate(),
                    ),
  ...
  Future<void> _fetchSuggestions(String query) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (var p in prioMatches) { results.add({'tekst': p, 'forslagstekst': p, 'isPrio': true}); }
        for (var item in data) { results.add(item); }
        setState(() { _suggestions = results; });
        _showOverlay();
      }
    } catch (e) { debugPrint("DAWA fejl: $e"); }
  }
    ...
    return OverlayEntry(
      builder: (context) => Positioned(
        ...
        child: CompositedTransformFollower(
          link: _layerLink, offset: Offset(0.0, renderBox.size.height + 5.0),
          child: Material(
            elevation: 8.0, borderRadius: BorderRadius.circular(12), color: Colors.white,
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return ListTile(
                  leading: Icon(isPrio ? Icons.train : Icons.location_on, size: 18, color: isPrio ? const Color(0xFF6366F1) : Colors.grey),
                  title: Text(item['tekst'], style: TextStyle(fontSize: 14, fontWeight: isPrio ? FontWeight.bold : FontWeight.normal)),
              ...
              children: [
                ...
                Focus(
                  onFocusChange: (hasFocus) {
                    ...
                    if (!hasFocus) Future.delayed(const Duration(milliseconds: 200), _removeOverlay);
                  },
                  child: TextField(
                    ...
                    decoration: InputDecoration(border: InputBorder.none, isDense: true, contentPadding: EdgeInsets.zero, hintText: widget.hint, hintStyle: TextStyle(color: Colors.grey.shade400)),
...
class RidesTab extends StatefulWidget {
  ...
  const RidesTab({
    super.key, 
    this.filterOrigin, this.filterDest, 
    this.originLat, this.originLng,
    this.destLat, this.destLng,
    required this.radius, this.filterDate, required this.onClearFilters
  });
...
class _RidesTabState extends State<RidesTab> {
  ...
  void didUpdateWidget(covariant RidesTab old) { 
    ...
    if (old.filterOrigin != widget.filterOrigin || old.filterDest != widget.filterDest || old.radius != widget.radius) {
      _displayDate = widget.filterDate ?? DateTime.now();
      _fetchRides();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (DateTime(now.year, now.month, now.day) == DateTime(_displayDate.year, _displayDate.month, _displayDate.day)) headerDateText = "I dag";
    else if (DateTime(now.year, now.month, now.day).add(const Duration(days: 1)) == DateTime(_displayDate.year, _displayDate.month, _displayDate.day)) headerDateText = "I morgen";
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          Expanded(
            child: _isLoading ? const Center(child: CircularProgressIndicator()) : _rides.isEmpty ? const Center(child: Text("Ingen lift fundet")) : ListView.builder(
              padding: const EdgeInsets.all(20),
              itemCount: _rides.length,
              itemBuilder: (context, index) => _buildRideCard(_rides[index], primaryColor),
            ),
          ),
        ],
      ),
  ...
  Widget _buildRideCard(Map<String, dynamic> ride, Color primaryColor) {
    final depTime = DateTime.parse(ride['departure_time']);
          ...
          child: Column(
            ...
            children: [
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                Text("${DateFormat('dd/MM/yyyy').format(depTime)} • ${DateFormat('HH:mm').format(depTime)}", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: primaryColor)),
                Text("${ride['price_dkk']} kr.", style: TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: primaryColor)),
              ]),
              const SizedBox(height: 20),
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Column(children: [
                  Container(width: 10, height: 10, decoration: BoxDecoration(border: Border.all(color: accentColor, width: 2), shape: BoxShape.circle)),
              ...
              Row(children: [
                CircleAvatar(radius: 16, backgroundImage: driver['avatar_url'] != null ? NetworkImage(driver['avatar_url']) : null, child: driver['avatar_url'] == null ? Text(driver['full_name'][0]) : null),
                const SizedBox(width: 10),
                Text(driver['full_name'], style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 14)),
                const Spacer(),
                if (ride['instant_booking'] == true) const Icon(Icons.bolt, color: Colors.amber, size: 20),
                Builder(builder: (context) {
                  try {
                    final car = jsonDecode(ride['car_model']);
                    return Text(car['make'] ?? 'Bil', style: TextStyle(fontSize: 12, color: Colors.grey[600]));
                  } catch (_) {
                    return Text(ride['car_model'] ?? 'Bil', style: TextStyle(fontSize: 12, color: Colors.grey[600]));
                  }
                }),
              ]),
            ],
